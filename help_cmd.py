...
class CogSelecter(discord.ui.Select["BaseView"]):
    ...
    def __init__(self, parent: BaseView, cogs: List[commands.Cog]) -> None:
        self.parent: BaseView = parent
        self._cog_mapping: Mapping[str, commands.Cog] = {
...
class HelpCog(BaseView):
    ...
    def __init__(self, cog: commands.Cog, **kwargs: Any) -> None:
        self.cog: commands.Cog = cog

        super().__init__(**kwargs)

        for chunk in grouper(20, cog.get_commands()):
            self.add_item(CommandSelecter(parent=self, cmds=chunk))
...
class HelpView(BaseView):
    ...
    Parameters
    ----------
    cogs: List[:class:`commands.Cog`]
        A list of cogs to display in the help view.
    """

    __slots__: Tuple[str, ...] = ()

    def __init__(self, cogs: List[commands.Cog], **kwargs: Any) -> None:
        super().__init__(**kwargs)
    ...
    def embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            title="Bot Help Menu",
            description="Hello, I'm <@514153552789372929>! A bot that does some useful things.",
        )
        embed.set_author(
...
class Help(commands.HelpCommand):
    ...
    async def _filter_mapping(
        self, mapping: Mapping[Optional[commands.Cog], List[CommandType]]
    ) -> Mapping[commands.Cog, List[CommandType]]:
        """An internal helper method to filter all commands."""
        cmds = sum(mapping.values(), [])
        await self.filter_commands(cmds)

        cogs: Dict[commands.Cog, List[CommandType]] = {}
        for command in cmds:
            if not command.cog:
                continue
    ...
    async def send_bot_help(self, mapping: Mapping[Optional[commands.Cog], List[CommandType]]) -> discord.Message:  # type: ignore
        """|coro|
        A method used to send the bot's main help message.
        Parameters
        ----------
        mapping: Mapping[Optional[:class:`commands.Cog`], List[:class:`commands.Command`]]
            A mapping of :class:`commands.Cog` to its list of commands.
        Returns
        -------
    ...
    async def send_cog_help(self, cog: commands.Cog, /) -> discord.Message:  # type: ignore
        """|coro|
