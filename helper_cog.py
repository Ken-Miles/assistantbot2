...
from aidenlib.main import getorfetch_user, makeembed
import datetime
import traceback
import pkgutil
import os
...
class BasicCog(commands.Cog):
    ...
    async def dmhistory(self, ctx: commands.Context, user: Optional[discord.User]=None, id: Optional[str]=None):
        ...
        user_ = user if user else user_
        if user_.dm_channel is None:
            await ctx.reply("User does not have a dm channel.")
            ch = await user_.create_dm()
        else:
            ch = user_.dm_channel
            #return
        msgs = [msg async for msg in ch.history(limit=None)]
        msgs.reverse()
        # returnv = ""
        # for msg in msgs:
